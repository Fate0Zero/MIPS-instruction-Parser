...
// Invocation options:
//
//    parse <asm code file> <results file name> [-prof]
//       param1:  name of file containing MIPS32 assembly instructions,
//                one per line; created by driver.c -f -prof is specified
//       param2:  name of file to hold results from parser
//       param3:  (optional) if present, random valid assembly instructions will
//                be generated and written to the specified input file, before
//                the parser is executed;
//                if omitted, a (presumably) preexisting input file will be used
//
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#include "Grading.h"           // for comparison/scoring logic
...
int main(int argc, char** argv) {
   ...
   if ( argc < 3 || argc > 4 ) {
      printf("Invocation:  parse <assembly instruction file> <parser results file> [-rand]\n");
      return 1;
   }
   // Check for -rand switch
   if ( argc == 4 ) {
